...
import {renderGame, createNewEl, checkIsMove} from './tool'
...
export function moveUpOrDown(direction, obj) {
  ...
  function checkNext() {
    ...
    if (flag) {
      ...
      // flag: 检查结果
      if (!flag) { // 说明检查结果为空，说明目标位置为空
        // console.warn('目标位置为空, row: ', y);
        let flag2 = null;
        // 再次检查是不是最后一行
        ...
        // 说明目标位置不为空
        // console.warn('目标位置不为空, row: ', y, 'column: ', x);
        let to = check({x, y});
        console.warn('目标位置为：', to, val);
        if (to.value === val) {
          // console.warn('val一样');
          // false不能移动
          if (checkIsMove('y', obj.y)) {
            moveToY(obj, to, {direction});
          } else {
            data[y][x].isMoveY = true;
            // console.error('走了')
            if (direction === 'up') {
          ...
          // console.error('检测是否可以移动', checkIsMove('y', obj.y));
  ...
  function checkNext() {
    ...
    if (flag) {
      ...
      if (!ret) {
        ...
        // 目标位置不为空
        console.warn('目标位置不为空, 移动格子信息：', obj, '目标格子信息：', ret);
      ...
      if (!ret) {
        ...
        if (val === ret.value) {
          ...
          data[y][x].isMoveX = true;
          if (checkIsMove('x', obj.x)) {
            moveToX(obj, ret, {direction});
          } else {
            if (direction === 'left') {
              x++;
